...
(defn line! [^IntBuffer ib x0 y0 x1 y1 c]
  (let [dx (Math/abs (int (- x1 x0)))
        sx (if (< x0 x1) 1 -1)
        dy (Math/abs (int (- y1 y0)))
        sy (if (< y0 y1) 1 -1)]
    (loop [x x0 y y0 err (quot (if (> dx dy) dx (- dy)) 2)]
      (put! ib x y c)
      (when (or (not= x x1) (not= y y1))
        (let [e2 err
              [err x] (if (> e2 (- dx)) [(- err dy) (+ x sx)] [err x])
              [err y] (if (< e2 dy)     [(+ err dx) (+ y sy)] [err y])]
          (recur x y err))))))

(defn rect! [^IntBuffer ib x y w h c]
  (dotimes [row h]
    (dotimes [col w]
      (put! ib (+ x col) (+ y row) c))))

(defn circle! [^IntBuffer ib cx cy r c]
  (dotimes [dy (inc (* 2 r))]
    (let [yy (- dy r)]
      (dotimes [dx (inc (* 2 r))]
        (let [xx (- dx r)]
          (when (<= (+ (* xx xx) (* yy yy)) (* r r))
...
(defn flood-fill! [^IntBuffer ib x y old c]
  (when (and (<= 0 x) (< x W) (<= 0 y) (< y H)
    ...
    (let [^ArrayDeque stack (ArrayDeque.)]
      (.addLast stack (int-array [x y]))
      (while (not (.isEmpty stack))
        (let [p (.removeLast stack)
              px (aget ^ints p 0)
              py (aget ^ints p 1)]
          (when (and (<= 0 px) (< px W) (<= 0 py) (< py H)
                     (= (get! ib px py) old))
            (put! ib px py c)
            (.addLast stack (int-array [(dec px) py]))
            (.addLast stack (int-array [(inc px) py]))
...
(def font5x3
  (int-array
    [0x0000,0x2092,0x002d,0x5f7d,0x279e,0x52a5,0x7ad6,0x0012,0x4494,0x1491,0x017a,0x05d0,0x1400,0x01c0,0x0400,0x12a4
     0x2b6a,0x749a,0x752a,0x38a3,0x4f4a,0x38cf,0x3bce,0x12a7,0x3aae,0x49ae,0x0410,0x1410,0x4454,0x0e38,0x1511,0x10e3
     0x73ee,0x5f7a,0x3beb,0x624e,0x3b6b,0x73cf,0x13cf,0x6b4e,0x5bed,0x7497,0x2b27,0x5add,0x7249,0x5b7d,0x5b6b,0x3b6e
     0x12eb,0x4f6b,0x5aeb,0x388e,0x2497,0x6b6d,0x256d,0x5f6d,0x5aad,0x24ad,0x72a7,0x6496,0x4889,0x3493,0x002a,0xf000
     0x0011,0x6b98,0x3b79,0x7270,0x7b74,0x6750,0x95d6,0xb9ee,0x5b59,0x6410,0xb482,0x56e8,0x6492,0x5be8,0x5b58,0x3b70
     0x976a,0xcd6a,0x1370,0x38f0,0x64ba,0x3b68,0x2568,0x5f68,0x54a8,0xb9ad,0x73b8,0x64d6,0x2492,0x3593,0x03e0]))

(defn text! [^IntBuffer ib x0 y0 ^String s scale c]
  (loop [x x0 i 0]
    (when (< i (.length s))
      (let [chr (int (.charAt s i))]
        (when (> chr 32)
          (let [bmp (aget ^ints font5x3 (- chr 32))]
            (dotimes [dy 5]
              (dotimes [dx 3]
                (when (pos? (bit-and (unsigned-bit-shift-right bmp (+ (* dy 3) dx)) 1))
